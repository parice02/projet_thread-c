 ...
 */
void thread_at_works(int thread_id, char *gender)
{
...
void *man_thread(void *arg)
{
    ...
    while (1)
    {
        thread_at_works(thread_man->id_thread, gender);
...
void *woman_thread(void *arg)
{
    ...
    while (1)
    {
        thread_at_works(thread_woman->id_thread, gender);
...
void enter_bathroom(t_thread *thread, char *gender)
{
    ...
    else if ((thread->bathroom->sex_in_bathroom == thread->sexe) && (nbre_cle_libre != 0))
    {
        ...
        if ((thread->sexe == HOMME) && (thread->en_attente->nbre_woman != 0))
        {
...
void leave_bathroom(t_thread *thread, char *gender)
{
    ...
    thread->bathroom->nbre_occupant--;
    fprintf(stdout, "Il reste %d %s(s) dans les toilettes, %d homme(s) et %d femme(s) en attente.\n", thread->bathroom->nbre_occupant, gender, thread->en_attente->nbre_man, thread->en_attente->nbre_woman);
    ...
    {
        pthread_mutex_lock(&thread->bathroom->check_condition);
        if ((thread->sexe == FEMME) && (thread->en_attente->nbre_woman != 0))
        {
            pthread_cond_signal(&thread->bathroom->woman_condition);
            thread->en_attente->nbre_woman--;
        }
        else if ((thread->sexe == HOMME) && (thread->en_attente->nbre_woman == 0))
        {
